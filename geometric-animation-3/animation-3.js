...
class FlowFieldEffect {
    ...
    #drawLine(angle, x, y) {
        ...
        if (distance > 5000000) distance = 6000000; //caps max distance length
        else if (distance < 500000) distance = 500000; //caps minimum distance length
        ...
        if (this.timer > this.interval) {
            ...
            for (let y = 0; y < this.#height; y += this.cellSize) {
                for (let x = 0; x < this.#width; x += this.cellSize) {
                    const angle = (Math.cos(x * 0.01) + Math.sin(y * 0.01)) * this.radius; //sets pattern like (x*0.01, y*-0.02), (x*0.01, y*0.005), (x*0.01,y*-0.01)
